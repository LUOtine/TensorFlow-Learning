...
def add_layer(inputs,raw,column,activation_function=None):
    Weight = tf.Variable(tf.truncated_normal([raw,column],stddev=0.1))
...
#add layer
l1=add_layer(x,784,700,activation_function=tf.nn.tanh)
l2=add_layer(l1,700,196,activation_function=tf.nn.relu)
prediction=add_layer(l2,196,10,activation_function=tf.nn.softmax)
#cross entropy
